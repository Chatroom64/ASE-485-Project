...
export function exportScore(score: Score): ScoreFile {
    const staffNum = score.staves[0];
    if (!staffNum) throw new Error("Staves not found");
...
export function importScore(data: ScoreFile): Score {
  ...
  for (const measureData of data.measures) {
    ...
    for (const el of measureData.elements) {
      ...
      if (el.type === "note") {
        measure.addElement(
          new Note(el.pitch, el.octave, el.duration)
        );
      }

      if (el.type === "rest") {
        measure.addElement(
          new Rest(el.duration)
        );
      }
    }
...
export function loadScoreFromFile(file: File): Promise<Score> {
